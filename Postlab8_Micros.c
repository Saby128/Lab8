...
char UNI;      // Declaraci?n de variables Unidades
char DECC;     // Variable decenas
char CEN;      // Variable centenas
char ttmr0;    // Variable de bandera de timer
int POTENCIOMETRO;  
int SENAL;     // Primera se?al de voltaje
int SENAL2;    // Segunda se?al de voltaje 
...
void __interrupt() isr(void){
    if(INTCONbits.T0IF){        //Interrupci?n de timer0
        INTCONbits.T0IF = 0;
    ...
    while(1){
        if(ADCON0bits.GO == 0){
            if(ADCON0bits.CHS == 11)
                ...
                ADCON0bits.CHS = 11;
            __delay_us(50);
            ADCON0bits.GO = 1;      // Se repite la conversi?n
        }
        calculos();
    }
}
...
void calculos(void){
    SENAL = PORTD;          // Guardar valor del puerto D en se?al
    POTENCIOMETRO = (SENAL << 2);     // Mover bits, mutiplicar por 4
    SENAL2 = POTENCIOMETRO >> 1;        // Mover a la derecha para divir entre 2 
    
    CEN = SENAL2/100;                   // Operaci?n para obtener centenas
    DECC = (SENAL2-(CEN*100))/10;       // Operaci?n para decenas
    UNI = (SENAL2-(CEN*100+DECC*10));   // Operaci?n para unidades
    return;
}
...
void setup(void){
    ...
    OPTION_REGbits.T0CS = 0;  // bit 5  Selecci?n de oscilador usado, en este caso oscilador interno
    OPTION_REGbits.T0SE = 0;  // bit 4 Bits de pulsos
    OPTION_REGbits.PSA = 0;   // bit 3  Preescaler asignado para el timer0
    OPTION_REGbits.PS2 = 1;   // bits 2-0  Selecci?n de bits de preescaler
    OPTION_REGbits.PS1 = 1;
    OPTION_REGbits.PS0 = 1;
    TMR0 = 255;
    
    INTCONbits.T0IF = 0;        // Limpiamos bandera del timer0
    INTCONbits.T0IE = 1;        // Se habilita la interrupci?n del timer0
    ...
    __delay_us(50);             
    ADCON0bits.ADON = 1;        // Se habilita el m?dulo 
